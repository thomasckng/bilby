...
def check_if_injections_in_prior(injection_df, priors):
    """Check if injections inside prior ranges. If not, log a warning informing user.

    Parameters
    ----------
    injection_df : pd.DataFrame
        Dataframe of CBC injections.
    priors : bilby.prior.PriorDict
        Priors for analysing injections.

    Returns
    -------
    invalid_inj: List[int]
        List of invalid injection ids.

    """

    # Check that injection params is subset of prior params
    if not set(priors.keys()).issubset(set(injection_df.columns)):
        logger.debug(
            "Injection dataframe does not contain all parameters present in priors. Adding extra parameters."
        )
        injection_df = injection_df.copy()
        injection_df = bilby.gw.conversion.generate_all_bbh_parameters(injection_df)
        injection_df = bilby.gw.conversion.generate_all_bns_parameters(injection_df)

    in_prior = pd.DataFrame(index=injection_df.index)
    for prior_name, prior in priors.items():
        inj_param = injection_df[prior_name].values
        in_prior[f"in_{prior_name}_prior"] = (prior.minimum <= inj_param) & (
            inj_param <= prior.maximum
        )
    not_in_prior = in_prior[
        in_prior.isin([False]).any(axis=1)
    ]  # get all rows where inj_param outside prior range
    invalid_inj = []
    if len(not_in_prior) > 0:
        invalid_inj = list(not_in_prior.T.columns)
        logger.warning(
            f"The following injection id(s) have parameters outside your prior range: "
            f"{invalid_inj}"
        )
    return invalid_inj
